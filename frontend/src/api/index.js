class Api {
  ...
  getRecipes ({
    ...
    tags
      ...
      return fetch(
        `/api/recipes/?page=${page}&limit=${limit}${author ? `&author=${author}` : ''}${is_favorited ? `&is_favorited=${is_favorited}` : ''}${is_in_basket ? `&is_in_basket=${is_in_basket}` : ''}${tagsString}`,
        {
          method: 'GET',
          headers: {
            ...this._headers,
